...
const InventorySlot: React.ForwardRefRenderFunction<HTMLDivElement, SlotProps> = (
  ...
  const [{ isDragging }, drag] = useDrag<DragSource, void, { isDragging: boolean }>(
    () => ({
      ...
      canDrag,
        ...
        if (!monitor.didDrop() && isSlotWithItem(item) && inventoryType === 'player') {
          fetchNui('dropItem', { slot: item.slot });
        }
      },
    }),
    [inventoryType, item]
      ...
      drop: (source) => {
        ...
        switch (source.inventory) {
          ...
          case InventoryType.CRAFTING:
            ...
            break;
          default:
            // Auto-attach component to weapon
            if (isSlotWithItem(item) && source.item.name) {
              const sourceData = Items[source.item.name];
              const targetData = Items[item.name];
              if (sourceData?.component && targetData?.weapon) {
                if (!sourceData.compatibleWeapons || sourceData.compatibleWeapons.includes(item.name)) {
                  onUse({ name: source.item.name, slot: source.item.slot });
                  return;
                }
              }
            }
  ...
  const handleContext = (event: React.MouseEvent<HTMLDivElement>) => {
    ...
    // Allow context menu for player and shop items
